...
rd <- rsDriver(browser = 'firefox',port = as.integer(444))
remDr <- rd$client
remDr$open()
...
bttv_emotes_top <- data.frame(emote_name,emote_image)
bttv_emotes_top %>% write.csv("bttv_emotes_top.csv")


remDr$navigate("https://betterttv.com/emotes/trending")
...
bttv_emotes_trending <- data.frame(emote_name,emote_image)
bttv_emotes_trending %>% write.csv("bttv_emotes_trending.csv")
